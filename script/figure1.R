...
fig1c <- ggplot(selected_long, aes(x=Collected_Region,y = Num,fill=Taxo)) +
  geom_bar(stat = "identity",position = "fill",
           width = 0.8,colour = "black", size = 0.25,
           alpha = 1)+ coord_flip()+
  labs(x="",y="")+theme_bw()+scale_fill_manual(values =color)+
  ggtitle('Species composition (location)')+ theme(legend.position = 'none')
# Region
...
selected_long$Collected_Time <- factor(selected_long$Collected_Time,levels = c("2021-Winter",
                                                                               "2022-Spring",
                                                                               "2022-Summer",
                                                                               "2022-Autumn",
                                                                               "2022-Winter"))
fig1d <- ggplot(selected_long, aes(x=Collected_Time,y = Num,fill=Taxo)) +
  geom_bar(stat = "identity",position = "fill",
           width = 0.8,colour = "black", size = 0.25,
           alpha = 1)+ coord_flip()+
  labs(x="",y="")+theme_bw()+scale_fill_manual(values =color)+
...
dddeee
"
Combine <- Maps+fig1b+fig1c+fig1d+plot_layout(design = layout)+
